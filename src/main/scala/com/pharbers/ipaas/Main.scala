...
import java.io.{BufferedReader, File, FileInputStream, InputStreamReader, RandomAccessFile}
import java.net.URI

import org.apache.hadoop.conf.Configuration
import org.apache.hadoop.fs.{FSDataInputStream, FileSystem, Hdfs, Path}
...
object Main {
    ...
    def main(args: Array[String]): Unit = {
        if(args.isEmpty) throw new Exception("args is empty")
        println("start main")

        val fis = FileSystem.get(URI.create(args(0)), new Configuration)
                .open(new Path(args(0))).getWrappedStream

        implicit val sd: PhSparkDriver = PhSparkDriver("test-driver")
        val phJobs = YamlInput()
                .readObjects[Job](fis)
                .map(x => getMethodMirror(x.getFactory)(x).asInstanceOf[PhFactoryTrait[PhJobTrait]].inst())
        val result = phJobs.head.perform(PhMapArgs(Map(
            "sparkDriver" -> PhSparkDriverArgs(sd),
            "logDriver" -> PhLogDriverArgs(PhLogDriver(formatMsg("test_user", "test_traceID", "test_jobID")))
        )))
        result.toMapArgs[PhDFArgs].get("cleanResult").get.show(false)
        //        Runner.run(args)
    }
}
