...
class TestSortOperator extends FunSuite with BeforeAndAfterAll {
    ...
    test("sort by VALUE as desc") {
        val operator = SortOperator(
            "SelectOperator",
            PhMapArgs(Map(
                "inDFName" -> PhStringArgs("inDFName"),
                "sortList" -> PhStringArgs("VALUE"),
                "orderStr" -> PhStringArgs("desc")
            )),
            Seq()
        )
