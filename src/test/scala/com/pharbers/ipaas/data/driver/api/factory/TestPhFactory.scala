...
class TestPhFactory extends FunSuite with BeforeAndAfterAll {
    ...
    override def beforeAll(): Unit = {
        ...
        sub.setReference("com.pharbers.ipaas.data.driver.plugin.AddByWhen")
        sub.setArgs(Map("condition" -> "condition", "trueValue" -> "trueValue", "otherValue" -> "otherValue").asJava)
        require(sub != null)

        plugin = Plugin()
        plugin.setName("plugin")
        plugin.setFactory("com.pharbers.ipaas.data.driver.api.factory.PhPluginFactory")
        plugin.setReference("com.pharbers.ipaas.data.driver.plugin.AddByWhen")
        plugin.setArgs(Map("condition" -> "condition", "trueValue" -> "trueValue", "otherValue" -> "otherValue").asJava)
        plugin.setSub(sub)
        require(plugin != null)
        ...
        operator.setReference("com.pharbers.ipaas.data.driver.operators.addColumn")
